...
import (
	...
	"github.com/eduardolat/goptional/internal/assert"
)
...
func TestOptionalTime(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			assert.NoError(t, err)
			assert.Equal(t, tt.expected.Present, got.Present)
			assert.True(t, got.Value.Equal(tt.expected.Value))
		})
	}
}
...
func TestNone(t *testing.T) {
	...
	t.Run("struct type", func(t *testing.T) {
		...
		opt := None[TestStruct]()
		assert.False(t, opt.Present)
		assert.Equal(t, "", opt.Value.Field)
	})
}
...
func TestEdgeCases(t *testing.T) {
	...
	t.Run("unmarshal very long null-like string", func(t *testing.T) {
		...
		assert.NoError(t, err)
		assert.True(t, opt.Present)
		assert.Equal(t, "nullish", opt.Value)
	})
	...
	t.Run("unmarshal array of optionals", func(t *testing.T) {
		...
		assert.Equal(t, Optional[int]{Value: 1, Present: true}, result.Values[0])
		assert.Equal(t, Optional[int]{Present: false}, result.Values[1])
		assert.Equal(t, Optional[int]{Value: 3, Present: true}, result.Values[2])
		assert.Equal(t, Optional[int]{Present: false}, result.Values[3])
		assert.Equal(t, Optional[int]{Value: 5, Present: true}, result.Values[4])
	})
}
